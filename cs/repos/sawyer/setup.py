...
# C1 = A0 # WARN: not sure what this means
# T1 = A3 # WARN: not sure what this means
...
n = 5 # number of seconds to read gas sensor
m = 1 # number of seconds to read temperature sensor
s = 10 # number of seconds to read all sensors should be greater than n+m+1

Sf1 = 11.61 # nA/ppm change this to match your barcode!!!!

# unsigned long etime;

sesnor1 = SO2(C1, T1, Sf1) # Sensor Types are EtoH, H2S, CO, IAQ, SO2, NO2,
...
def loop():
  ...
  if (millis() - etime > (s * 1000)): # also handles case where millis rolls
    ...
    Serial.print(", ")
    Serial.println(sensor1.convertX('M')) # use 'M' or 'B' for units or
